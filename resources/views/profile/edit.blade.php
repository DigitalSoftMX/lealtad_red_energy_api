@extends('layouts.app', ['activePage' => 'Perfil', 'titlePage' => __('Perfil de usuario')])
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('first_surname') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('first_surname') ? ' is-invalid' : '' }}" name="first_surname" id="input-name" type="text" placeholder="{{ __('Apellido paterno') }}" value="{{ old('first_surname', auth()->user()->first_surname) }}" required="true" aria-required="true"/>
                      @if ($errors->has('first_surname'))
                        <span id="name-error" class="error text-danger" for="input-name">{{ $errors->first('first_surname') }}</span>
                      @endif
                    </div>
                  </div>
                </div>
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('second_surname') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('second_surname') ? ' is-invalid' : '' }}" name="second_surname" id="input-name" type="text" placeholder="{{ __('Apellido materno') }}" value="{{ old('second_surname', auth()->user()->second_surname) }}" required="true" aria-required="true"/>
                      @if ($errors->has('second_surname'))
                        <span id="name-error" class="error text-danger" for="input-name">{{ $errors->first('second_surname') }}</span>
                      @endif
                    </div>
                  </div>
                </div>
          ...
          <form method="post" action="{{ route('profile.password') }}" class="form-horizontal">
            ...
            <div class="card ">
              ...
              <div class="card-body ">
                ...
                <div class="row">
                  <label class="col-sm-2 col-form-label" for="input-password-confirmation">{{ __('Confirma nueva contraseña') }}</label>
                  <div class="col-sm-7">
